...
function Raqueta:actualizar(dt)
    ...
    if self._brillo > 0 then
        self._brillo = math.max(0, self._brillo - dt * 4) -- Se apaga en 0.25 segundos
    end
end
...
function Raqueta:dibujar()
    ...
    -- 0. Sombra paralela (Drop shadow) para dar realce y que no se vea plano
    love.graphics.setColor(0, 0, 0, 0.6)
    love.graphics.rectangle("fill", self._x + 4, self._y + 6, self._ancho, self._alto, radio, radio)
    
    -- 1. Sombra o borde exterior grueso (más oscuro)
